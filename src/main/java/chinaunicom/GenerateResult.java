...
public class GenerateResult {
	...
	public static Map<String, CrawlerResult> resultMap = null;
	
	public static Map<String, CrawlerResult> getResultTemplate() throws Exception
	{
		if(resultMap == null)
		{
			resultMap = parseXml();
	...
	/*public static void main(String[] args) {
		...
		try {
			Map<String, CrawlerResult> map = GenerateResult.parseXml();
			CrawlerResult template = map.get(ChinaunicomConstant.SMS);
...
//			System.out.println(obj.parseSroucedata(retJson, template));
	 ...
	 * @return
	 */
	public static int getPageTotal(JSONObject retJson, CrawlerPage page)
	{
...
//			json
		...
		JSONObject jsonObj = (JSONObject) getDataByPath(retJson, page.getPathes());
	...
	public static Map<String, Object> parseSroucedata(JSONObject retJson, CrawlerResult template)
	{
		...
		for(Entry<String, CrawlerDatasource> entry : sourceMap.entrySet())
		{
			...
			dataMap.put(sourceObj.getId(), getDataByPath(retJson, sourceObj.getPathes()));
		}
	...
	public static Object generateTargetdata(Map<String, Object> dataMap, CrawlerResult template)
	{
			...
			for(Entry<String, Object> entry : dataMap.entrySet())
			{
				...
				if(jsonArray == null)
				{
					...
					for(int i=0, size=temp.size(); i<size; i++)
					{
						...
						assembleAttribute(jsonObj, temp.getJSONObject(i), attributes);
					}
				}
				else
				{
...
//			jsonobject
	...
	public static void assembleAttribute(JSONObject targetObj, JSONObject sourceObj, 
	 ...
	 * @throws Exception
	 */
	public static Map<String, CrawlerResult> parseXml() throws Exception
	{
		SAXReader reader = new SAXReader();  
//        Document document = reader.read(new File("D:\\Workspace\\github\\crawler\\src\\main\\java\\crawlerresult.xml"));
		
		ClassLoader classLoader = GenerateResult.class.getClassLoader();
...
//		Document document = reader.read(new File("crawlerresult.xml"));
        ...
        Map<String, CrawlerResult> dataTemplates = new HashMap<>();
        CrawlerResult obj = null;
        for(Element infoEle : infoEles)
        {
        	obj = new CrawlerResult();
...
//        	拼装attribute
	 ...
	 * @return
	 */
	public static Object getDataByPath(JSONObject retJson, List<CrawlerPath> pathes)
	{
	 ...
	 * @return
	 */
	public static Object getDataByPath(Object targetObj, CrawlerPath path)
	{
...
//			jsonobject
	...
	public static String getValueByAttribute(JSONObject json, String sourceAttribute)
	{
		...
		if(StringUtils.isNotBlank(sourceAttribute))
		{
			...
			for(String str : arr)
			{
				...
				{
					buff.append(StringUtils.isNotBlank(val) ? val : "");
				}
			}
		}
	...
	public static Object generateResultObj(String retString, String entityKey)
	{
		try
		{
			if(StringUtils.isNotBlank(retString))
			{
				JSONObject json = JSON.parseObject(retString);
				System.out.println("返回结果json:::::"+JSON.toJSONString(json));
				CrawlerResult template = getResultTemplate().get(entityKey);
				return generateTargetdata(parseSroucedata(json, template), 
						template);
			}
		}
		catch(Exception e)
	...
	public static int getPageTotal(String retString, String entityName)
	{
		try
		{
			return getPageTotal(JSON.parseObject(retString),
					resultMap.get(entityName).getPage());
		}
		catch(Exception e)
		{
//			e.printStackTrace();
